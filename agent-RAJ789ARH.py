...
class DQLearning(QLearning):
    ...
    def display_policy(self, path1, path2):
        """
        Test agent with a given policy
        """
        print("Testing Agent...")
        ...
        for i in range(1, 100+1):
            ...
            observation, done = self.env.reset()
            pursuer_states = []
            pursuer_states.append(observation)
            ugs_states = []
            ugs_states.append(self.env.UGS[(observation[0],observation[1])])
            evader_states = []
            evader_states.append((self.env.evader.x, self.env.evader.y))
            col_actions = []
            while not done:
                # take a step
                # Get action from average of Q-Tables
                ...
                new_observation, _, done, episode_status = self.env.step(action, observation)
                self.env.render()
                pursuer_states.append(new_observation)
                ugs_states.append(self.env.UGS[(new_observation[0],new_observation[1])])
                evader_states.append((self.env.evader.x, self.env.evader.y))
                # update observation
                observation = new_observation
            # update success count
            self.env.close("results\\Erroneous_Information_0.1\\DQLearning\\sigmoid\\1706240875\\episodes_figures", i)
            if episode_status:
                success_count += 1
            if (observation[0],observation[1]) != (self.env.evader.x, self.env.evader.y):
                print(f"Episode {i} Start")
                print("Pursuer States", pursuer_states)
                print("UGS States", ugs_states)
                print("Evader States", evader_states)
                print("Actions", col_actions)
                print("Escaped")
                print(f"Episode {i} End\n")
        print(f"Done Testing Agent with {success_count*100/100}% success rate")
